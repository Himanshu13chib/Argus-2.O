...
class PredictiveAnalyticsEngine:
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            self.models = {'crossing_prediction': None, 'hardware_failure': None}
            self.anomaly_detectors = {'crossing_patterns': None, 'behavioral': None}
            self.scalers = {'crossing_features': None, 'hardware_metrics': None, 'environmental': None}
    ...
    async def detect_anomalous_behavior(self, recent_crossings: List[Dict]) -> AnomalyDetection:
        ...
        try:
            ...
            # Simple rule-based anomaly detection (fallback if ML models not available)
            if self.anomaly_detectors['crossing_patterns'] is None:
                ...
                for crossing in recent_crossings:
                    crossing_time = pd.to_datetime(crossing['timestamp'])
                    # Check for unusual times (3-5 AM)
                    if 3 <= crossing_time.hour <= 5:
                        anomaly_score += 0.3
                    # Check for large groups
                    if crossing.get('group_size', 1) > 3:
                        anomaly_score += 0.4
                    # Check for very high speed
                    if crossing.get('speed', 0.0) > 2.5:
                        anomaly_score += 0.2
                ...
                try:
                    ...
                    anomaly_score = float(abs(np.mean(anomaly_scores)))
                    is_anomaly = bool(np.any(anomaly_predictions == -1))
                except Exception as e:
                    logger.warning(f"ML anomaly detection failed, using rule-based: {e}")
                    anomaly_score = 0.1
                    is_anomaly = False
            ...
            if is_anomaly:
                ...
                description = f"Detected {anomaly_type} in crossing behavior with score {anomaly_score:.3f}"
            else:
                severity = "low"
                anomaly_type = "normal"
    ...
    async def predict_hardware_maintenance(self, hardware_metrics: Dict) -> PredictionResult:
        ...
        try:
            ...
            # Rule-based prediction (works without ML models)
...
if __name__ == "__main__":
    print("Starting Predictive Analytics Service...")
    try:
        uvicorn.run(
            "main_working:app",
            host="0.0.0.0",
        ...
        print("Service initialized successfully but server not started")